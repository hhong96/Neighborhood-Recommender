...
def cbsa_input(user_cbsa):
  family_type = data_read("select distinct family_type_dummy, case when family_type_dummy = 1 then 'Yes' else 'No' end as family_type_final from listings_enriched_final order by family_type_dummy desc")
  cent = data_read("select distinct centrality_index_dummy from listings_enriched_final order by centrality_index_dummy")
  yearbuilt = data_read("select * from call_yearbuilt order by yearbuilt_buckets_dummy desc")
  bd = data_read("select distinct bedroom_bucket_dummy, cast(round(bedrooms_final) as char) as bedrooms_final from listings_enriched_final  group by bedroom_bucket_dummy, bedrooms_final order by bedroom_bucket_dummy")
  sqft = data_read("select * from call_sqft")
  hometype = data_read("select * from call_hometype")
  income = data_read("select * from call_income")
...
# %%    
  ...
  if cbsa_param:
    ...
    with col3:
      ...
      centrality_param = st.select_slider("Do you prefer city?", options=cent['centrality_index_dummy'].tolist(), on_change=None)  
